...
# Generate .git_sha file when git metadata is available.
# Railway build contexts may omit .git, so keep the image buildable there too.
RUN if git rev-parse HEAD >/tmp/git_sha 2>/dev/null; then \
  cat /tmp/git_sha > /app/.git_sha; \
  else echo local-dev > /app/.git_sha; \
  fi
